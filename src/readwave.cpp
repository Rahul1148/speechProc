...
bool WaveRewind(FILE *wav_file, WavFileHead *wavHead)
{
	char riff[8],wavefmt[8];
	short i;
	rewind(wav_file);
	fread(wavHead,sizeof( WavFileHead),1,wav_file);
...
{                                                               
	...
	waveData = new char[(File_length-sizeof( WavFileHead))/sizeof(char)];
	numRead = fread(waveData, sizeof(char), File_length-sizeof( WavFileHead), wavFp);
	fclose(wavFp);
...
{                                                               
    ...
    int output = (File_length-sizeof( WavFileHead))/sizeof(short);
    printf("%d \n", output);
	waveData = new short [(File_length-sizeof( WavFileHead))/sizeof(short)];
	numRead = fread(waveData, sizeof(short), (File_length-sizeof( WavFileHead))/sizeof(short), wavFp);
	fclose(wavFp);
